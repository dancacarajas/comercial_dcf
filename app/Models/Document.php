...
final class Document extends Model
{
    ...
    /** @return array<int, array<string, mixed>> */
    public function filterFinancialEntryOptions(): array
    {
        return $this->query(
            'SELECT fe.`id`, CONCAT(fe.`title`, " — ", COALESCE(sp.`sponsor_display_name`, "")) AS label
               FROM `financial_entries` fe
               LEFT JOIN `sponsors` sp ON sp.`id` = fe.`sponsor_id`
              WHERE fe.`archived_at` IS NULL
              ORDER BY fe.`title` ASC
              LIMIT 300'
        )->fetchAll();
    }
    ...
    /** @return array<int, array<string, mixed>> */
    public function findByFinancialEntry(int|string $financialEntryId, int $limit = 10): array
    {
        return $this->findByFk('financial_entry_id', $financialEntryId, $limit);
    }

    public function countByFinancialEntry(int|string $financialEntryId): int
    {
        return $this->countByFk('financial_entry_id', $financialEntryId);
    }

    /** @return array{total:int,active:int,expired:int,expiring_soon:int} */
    public function summaryByFinancialEntry(int|string $financialEntryId): array
    {
        return $this->summaryByFk('financial_entry_id', $financialEntryId);
    }
