...
final class Document extends Model
{
    ...
    private const FILLABLE = [
        'company_id', 'contact_id', 'opportunity_id', 'quota_id', 'proposal_id', 'lead_id', 'sponsor_id', 'counterpart_id', 'contract_id',
    ...
    private const LIST_COLUMNS = '
        d.`id`, d.`company_id`, d.`contact_id`, d.`opportunity_id`, d.`quota_id`, d.`proposal_id`, d.`lead_id`, d.`sponsor_id`, d.`counterpart_id`, d.`contract_id`,
    ...
    private function buildWhere(array $filters): array
    {
        ...
        foreach (['company_id', 'contact_id', 'opportunity_id', 'quota_id', 'proposal_id', 'lead_id', 'sponsor_id', 'counterpart_id', 'contract_id', 'responsible_user_id'] as $fk) {
            $v = (int) ($filters[$fk] ?? 0);
            if ($v > 0) {
                $conditions[] = 'd.`' . $fk . '` = :' . $fk;
                $params[$fk]  = $v;
    ...
    /** @return array<int, array<string, mixed>> */
    public function filterCounterpartOptions(): array
    {
        return $this->query(
            'SELECT cp.`id`, CONCAT(cp.`title`, " — ", COALESCE(sp.`sponsor_display_name`, "")) AS label
               FROM `counterparts` cp
               LEFT JOIN `sponsors` sp ON sp.`id` = cp.`sponsor_id`
    ...
    /** @return array<int, array<string, mixed>> */
    public function filterContractOptions(): array
    {
        return $this->query(
            'SELECT ct.`id`, CONCAT(ct.`title`, " — ", COALESCE(sp.`sponsor_display_name`, "")) AS label
               FROM `contracts` ct
               LEFT JOIN `sponsors` sp ON sp.`id` = ct.`sponsor_id`
    ...
    /** @return array<int, array<string, mixed>> */
    public function findByCounterpart(int|string $counterpartId, int $limit = 10): array
    {
        return $this->findByFk('counterpart_id', $counterpartId, $limit);
    }

    public function countByCounterpart(int|string $counterpartId): int
    {
        return $this->countByFk('counterpart_id', $counterpartId);
    }

    /** @return array{total:int,active:int,expired:int,expiring_soon:int} */
    public function summaryByCounterpart(int|string $counterpartId): array
    {
        return $this->summaryByFk('counterpart_id', $counterpartId);
    }

    /** @return array<int, array<string, mixed>> */
    public function findByContract(int|string $contractId, int $limit = 10): array
    {
        return $this->findByFk('contract_id', $contractId, $limit);
    }

    public function countByContract(int|string $contractId): int
    {
        return $this->countByFk('contract_id', $contractId);
    }

    /** @return array{total:int,active:int,expired:int,expiring_soon:int} */
    public function summaryByContract(int|string $contractId): array
    {
        return $this->summaryByFk('contract_id', $contractId);
    }
    ...
    {
        if (!in_array($column, ['company_id', 'contact_id', 'opportunity_id', 'quota_id', 'proposal_id', 'lead_id', 'sponsor_id', 'counterpart_id', 'contract_id'], true)) {
            return [];
        }
    ...
    private function prepareRow(array $data, bool $isCreate = true): array
    {
        ...
        foreach (['company_id', 'contact_id', 'opportunity_id', 'quota_id', 'proposal_id', 'lead_id', 'sponsor_id', 'counterpart_id', 'contract_id',
