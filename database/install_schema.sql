...
INSERT IGNORE INTO `role_permissions` (`role_id`, `permission_id`)
SELECT r.`id`, p.`id` FROM `roles` r JOIN `permissions` p ON p.`slug` IN (
    'collector_assignments.view', 'collector_deals.view'
) WHERE r.`slug` IN ('producao-coordenacao', 'comunicacao', 'leitura-consulta');
...
-- =====================================================================
-- ETAPA 19 — Projetos Incentivados / PRONACs / Plano de Captação
-- =====================================================================
...
-- ETAPA 20A — Motor de Comissão dos Captadores
CREATE TABLE IF NOT EXISTS `commission_pools` (
    `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    `incentive_project_id` BIGINT UNSIGNED NOT NULL,
...
INSERT INTO `permissions` (`name`, `slug`, `description`) VALUES
    ('Comissões: visualizar', 'commissions.view', 'Visualizar motor de comissões dos captadores'),
    ('Comissões: calcular',   'commissions.calculate', 'Calcular/recalcular comissões dos captadores'),
    ('Comissões: aprovar',    'commissions.approve', 'Reservado para etapa de aprovação'),
    ('Comissões: bloquear',   'commissions.block', 'Reservado para bloqueio interno')
ON DUPLICATE KEY UPDATE `name` = VALUES(`name`), `description` = VALUES(`description`);
...
INSERT IGNORE INTO `role_permissions` (`role_id`, `permission_id`)
SELECT r.`id`, p.`id` FROM `roles` r JOIN `permissions` p ON p.`slug` IN ('commissions.pay','commissions.cancel_payment')
WHERE r.`slug` = 'administrador-geral';
