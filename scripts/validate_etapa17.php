...
/**
 * Validação local HTTP — Etapa 17 Relatórios Avançados / Indicadores Gerenciais
 * Executar: docker exec dcc_app php /var/www/html/scripts/validate_etapa17.php
 */
...
function ensureSemReportsUser(): void
{
    $row = dbq('SELECT id FROM users WHERE email = ?', ['sem-reports@test.com']);
    if (!$row) {
        ...
        )->execute(['Sem Relatórios', 'sem-reports@test.com', $hash, 'active']);
...
function ensureSemSnapshotsUser(): void
{
    $row = dbq('SELECT id FROM users WHERE email = ?', ['sem-snapshots@test.com']);
    if (!$row) {
        $hash = dbq('SELECT password_hash FROM users WHERE id = 1')['password_hash'];
        db()->prepare(
            'INSERT INTO users (name, email, password_hash, status, created_at) VALUES (?, ?, ?, ?, NOW())'
        )->execute(['Sem Snapshots', 'sem-snapshots@test.com', $hash, 'active']);
        $uid = (int) db()->lastInsertId();
        ...
        db()->prepare('UPDATE users SET status = ? WHERE email = ?')->execute(['active', 'sem-snapshots@test.com']);
    }
}
...
function archiveTestData(Report $R): void
{
    ...
    db()->prepare('UPDATE users SET status=? WHERE email IN (?, ?)')->execute(['inactive', 'sem-reports@test.com', 'sem-snapshots@test.com']);
    $R->pass('limpeza', 'Snapshots de teste arquivados / usuários inativados', "snapshots={$sn}");
}

echo '=== VALIDAÇÃO ETAPA 17 — RELATÓRIOS / INDICADORES GERENCIAIS ===' . PHP_EOL . PHP_EOL;
$R = new Report();
...
ensureSemReportsUser();
$sem = new HttpClient();
$sem->login('sem-reports@test.com', PASSWORD);
$r = $sem->get('/reports');
($r['code'] === 403)
...
ensureSemSnapshotsUser();
$semSnap = new HttpClient();
$semSnap->login('sem-snapshots@test.com', PASSWORD);
...
echo PHP_EOL . '11. ACTIVITY LOGS (OPCIONAL)' . PHP_EOL;
