...
/**
 * Validação local HTTP — Etapa 14 Contratos / Instrumentos de Formalização
 * Executar: docker exec dcc_app php /var/www/html/scripts/validate_etapa14.php
 */
...
function ensureSemContractsUser(): void
{
    $row = dbq('SELECT id FROM users WHERE email = ?', ['sem-contracts@test.com']);
    if (!$row) {
        ...
        )->execute(['Sem Contratos', 'sem-contracts@test.com', $hash, 'active']);
...
ensureSemContractsUser();
$sem = new HttpClient();
$sem->login('sem-contracts@test.com', PASSWORD);
$r = $sem->get('/contracts');
