...
/**
 * Validação HTTP — Etapa 15 Financeiro Detalhado (produção)
 * Executar no servidor: php scripts/validate_etapa15_production.php
 */

const BASE_URL = 'https://comercial.dancacarajas.com.br';
const PASSWORD = 'Mudar@123';
const ADMIN_EMAIL = 'validacao-etapa15-admin@test.com';
const SEM_VIEW_EMAIL = 'validacao-etapa15-sem@test.com';
const LEITOR_EMAIL = 'validacao-etapa15-leitor@test.com';
const CAP_EMAIL = 'validacao-etapa15-cap@test.com';
const PROD_PREFIX = 'FINANCEIRO ETAPA 15 PRODUCAO';
...
function ensureTempValidationUsers(): void
{
    ...
    $users = [
        [ADMIN_EMAIL, 'Validacao Etapa15 Admin', 'administrador-geral'],
        [LEITOR_EMAIL, 'Validacao Etapa15 Leitor', 'leitura-consulta'],
        [CAP_EMAIL, 'Validacao Etapa15 Captacao', 'captacao-comercial'],
    ];
    ...
    $semRole = dbq("SELECT id FROM roles WHERE slug = 'teste-sem-financials-etapa15'");
    if (!$semRole) {
        db()->prepare('INSERT INTO roles (name, slug, description) VALUES (?, ?, ?)')
            ->execute(['Teste sem financials Etapa15', 'teste-sem-financials-etapa15', 'Role temporaria validacao']);
        $semRole = dbq("SELECT id FROM roles WHERE slug = 'teste-sem-financials-etapa15'");
    }
    $row = dbq('SELECT id FROM users WHERE email = ?', [SEM_VIEW_EMAIL]);
    if (!$row) {
        db()->prepare('INSERT INTO users (name, email, password_hash, status, created_at) VALUES (?, ?, ?, ?, NOW())')
            ->execute(['Validacao Etapa15 Sem', SEM_VIEW_EMAIL, $hash, 'active']);
        $uid = (int) db()->lastInsertId();
    } else {
        $uid = (int) $row['id'];
        db()->prepare("UPDATE users SET status='active', password_hash=? WHERE id=?")->execute([$hash, $uid]);
...
function ensureFullTestChain(HttpClient $http, Report $R): array
{
    ...
    $csrf = HttpClient::extractCsrf($page['body']);
    $res = $http->post('/companies', [
        '_csrf' => $csrf, 'name' => 'EMPRESA TESTE — ' . PROD_PREFIX, 'status' => 'prospect', 'priority' => 'B',
    ]);
    $companyId = 0;
    if (preg_match('#/companies/(\d+)#', (string) ($res['location'] ?? ''), $m)) {
        $companyId = (int) $m[1];
    } else {
        $companyId = (int) (dbq('SELECT id FROM companies WHERE name LIKE ? ORDER BY id DESC LIMIT 1', ['%EMPRESA TESTE — ' . PROD_PREFIX . '%'])['id'] ?? 0);
    }
    ...
    $res = $http->post('/contacts', [
        '_csrf' => $csrf, 'company_id' => (string) $companyId, 'name' => 'CONTATO TESTE — ' . PROD_PREFIX, 'status' => 'ativo',
    ]);
    $contactId = preg_match('#/contacts/(\d+)#', (string) ($res['location'] ?? ''), $m) ? (int) $m[1]
        : (int) (dbq('SELECT id FROM contacts WHERE name LIKE ? ORDER BY id DESC LIMIT 1', ['%CONTATO TESTE — ' . PROD_PREFIX . '%'])['id'] ?? 0);
    ...
    $res = $http->post('/opportunities', [
        ...
        'quota_id' => (string) $quotaId, 'title' => 'OPORTUNIDADE TESTE — ' . PROD_PREFIX,
        'status' => 'negociacao', 'estimated_value' => '50000', 'probability' => '50',
    ]);
    $oppId = preg_match('#/opportunities/(\d+)#', (string) ($res['location'] ?? ''), $m) ? (int) $m[1]
        : (int) (dbq('SELECT id FROM opportunities WHERE title LIKE ? ORDER BY id DESC LIMIT 1', ['%OPORTUNIDADE TESTE — ' . PROD_PREFIX . '%'])['id'] ?? 0);
    ...
    $res = $http->post('/proposals', [
        ...
        'title' => 'PROPOSTA TESTE — ' . PROD_PREFIX, 'type' => 'proposta_por_cota', 'proposed_value' => '50000', 'status' => 'rascunho',
    ]);
    $propId = preg_match('#/proposals/(\d+)#', (string) ($res['location'] ?? ''), $m) ? (int) $m[1]
        : (int) (dbq('SELECT id FROM proposals WHERE title LIKE ? ORDER BY id DESC LIMIT 1', ['%PROPOSTA TESTE — ' . PROD_PREFIX . '%'])['id'] ?? 0);
    ...
    $res = $http->post('/sponsors', [
        ...
        'sponsor_display_name' => 'PATROCINADOR TESTE — ' . PROD_PREFIX,
    ...
    $sponsorId = preg_match('#/sponsors/(\d+)#', (string) ($res['location'] ?? ''), $m) ? (int) $m[1]
        : (int) (dbq('SELECT id FROM sponsors WHERE sponsor_display_name LIKE ? ORDER BY id DESC LIMIT 1', ['%PATROCINADOR TESTE — ' . PROD_PREFIX . '%'])['id'] ?? 0);
    $sponsorId > 0 ? $R->pass('dados', 'Patrocinador cadeia completa', 'id=' . $sponsorId) : $R->fail('dados', 'Patrocinador cadeia');

    $contractTitle = 'CONTRATO TESTE — ' . PROD_PREFIX;
...
{
    echo PHP_EOL . "11. LIMPEZA PRODUÇÃO" . PHP_EOL;
    $now = date('Y-m-d H:i:s');
    $fe = db()->exec("UPDATE financial_entries SET archived_at='{$now}' WHERE archived_at IS NULL AND (title LIKE 'LANÇAMENTO TESTE ETAPA 15%' OR title LIKE 'PARCIAL ETAPA 15%' OR title LIKE 'TOTAL ETAPA 15%' OR title LIKE 'CANCEL ETAPA 15%' OR title LIKE '%FINANCEIRO ETAPA 15 PRODUCAO%')");
    $doc = db()->exec("UPDATE documents SET archived_at='{$now}' WHERE archived_at IS NULL AND (title LIKE '%ETAPA 15%' OR title LIKE '%FINANCEIRO PRODUCAO%')");
    $sp = db()->exec("UPDATE sponsors SET archived_at='{$now}' WHERE archived_at IS NULL AND (sponsor_display_name LIKE '%FINANCEIRO ETAPA 15 PRODUCAO%' OR sponsor_display_name LIKE 'PATROCINADOR BASE ETAPA 15%')");
    db()->exec("UPDATE companies SET archived_at='{$now}' WHERE archived_at IS NULL AND name LIKE '%FINANCEIRO ETAPA 15 PRODUCAO%'");
    db()->exec("UPDATE contacts SET archived_at='{$now}' WHERE archived_at IS NULL AND name LIKE '%FINANCEIRO ETAPA 15 PRODUCAO%'");
    db()->exec("UPDATE opportunities SET archived_at='{$now}' WHERE archived_at IS NULL AND title LIKE '%FINANCEIRO ETAPA 15 PRODUCAO%'");
    db()->exec("UPDATE proposals SET archived_at='{$now}' WHERE archived_at IS NULL AND title LIKE '%FINANCEIRO ETAPA 15 PRODUCAO%'");
    db()->exec("UPDATE contracts SET archived_at='{$now}' WHERE archived_at IS NULL AND title LIKE '%FINANCEIRO ETAPA 15 PRODUCAO%'");
    foreach ([ADMIN_EMAIL, SEM_VIEW_EMAIL, LEITOR_EMAIL, CAP_EMAIL] as $email) {
        db()->prepare('UPDATE users SET status=? WHERE email=?')->execute(['inactive', $email]);
    }
    $R->pass('limpeza', 'Dados de teste arquivados/usuários inativados', "fe={$fe} doc={$doc} sp={$sp}");
}
...
echo "=== VALIDAÇÃO ETAPA 15 — FINANCEIRO (PRODUÇÃO) ===" . PHP_EOL . PHP_EOL;
$R = new Report();

echo "0. SAÚDE E CONFIG" . PHP_EOL;
$health = (new HttpClient())->get('/health');
$healthOk = $health['code'] === 200 && str_contains($health['body'], '"status":"ok"');
$healthOk ? $R->pass('ambiente', '/health → ok') : $R->fail('ambiente', '/health', 'code=' . $health['code']);
$envFile = dirname(__DIR__) . '/.env';
$debugOff = !is_file($envFile) || !preg_match('/^\s*APP_DEBUG\s*=\s*true\s*$/mi', file_get_contents($envFile));
$debugOff ? $R->pass('ambiente', 'APP_DEBUG=false') : $R->fail('ambiente', 'APP_DEBUG deve ser false');

echo PHP_EOL . "1. AMBIENTE / BANCO" . PHP_EOL;
...
($leitor->get('/financials')['code'] === 200) ? $R->pass('auth', 'GET /financials leitor → 200') : $R->fail('auth', 'leitor list');
($leitor->get('/financials/create')['code'] === 403) ? $R->pass('auth', 'GET /financials/create leitor → 403') : $R->fail('auth', 'leitor create');
($leitor->post('/financials', ['_csrf' => 'x', 'sponsor_id' => '1', 'title' => 'X'])['code'] === 419 || $leitor->post('/financials', ['_csrf' => 'x', 'sponsor_id' => '1', 'title' => 'X'])['code'] === 403)
    ? $R->pass('auth', 'POST /financials leitor → 403/419')
    : $R->fail('auth', 'leitor POST');

$cap = new HttpClient();
$cap->login(CAP_EMAIL, PASSWORD);
($cap->get('/financials/create')['code'] === 200) ? $R->pass('auth', 'GET /financials/create captacao → 200') : $R->fail('auth', 'cap create');
...
if ($feId > 0) {
    ...
    if ($totalId > 0) {
        ...
        $showCap = $cap->get('/financials/' . $totalId);
        $csrfCap = HttpClient::extractCsrf($showCap['body']);
        ($cap->post('/financials/' . $totalId . '/archive', ['_csrf' => $csrfCap ?? ''])['code'] === 403)
            ? $R->pass('auth', 'POST archive captacao → 403')
            : $R->fail('auth', 'cap archive');
        ($cap->post('/financials/' . $totalId . '/reconcile', ['_csrf' => $csrfCap ?? ''])['code'] === 403)
            ? $R->pass('auth', 'POST reconcile captacao → 403')
            : $R->fail('auth', 'cap reconcile');
    } else {
    ...
    $proofTitle = 'COMPROVANTE TESTE — FINANCEIRO PRODUCAO';
    ...
    $receiptTitle = 'RECIBO TESTE — FINANCEIRO PRODUCAO';
    ...
    $fiscalTitle = 'DOCUMENTO FISCAL TESTE — FINANCEIRO PRODUCAO';
...
echo PHP_EOL . 'Validação Etapa 15 em produção concluída com sucesso.' . PHP_EOL;
